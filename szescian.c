...
#define r_dolne 4 // dolna podstawa stozka scietego
#define r_gorne 2 // gorna podstawa stozka scietego
#define wysokosc       10 // wysokosc podstawa stozka scietego
...
void rysujStozek()
{
  ...
  glBegin(GL_QUAD_STRIP);
    glColor3f(0, 0, 1);
    for(y = 0.0; y < wysokosc - 0.0001; y += 1.0 * wysokosc / M, poziom+=((R2-R1)/M))
	{
	  ...
	  for(fi = 0.0; fi <= 2.0001*PI; fi += 2*PI/N)
      {
        ...
        glVertex3f(x, y + 1.0 * wysokosc / M, z);
	  }
	}
  glEnd();
}
...
void Sfera() {
  ...
  for(alfa = 0; alfa <= PI-0.0001; alfa += PI/M)
  {
    glBegin(GL_TRIANGLE_STRIP);
    for(beta = 0.0; beta <= 1.5001*PI; beta += 1.5*PI/N)
    {
        x = promien * sin(alfa+PI/M) * cos(beta);
        z = promien * sin(alfa+PI/M) * sin(beta);
        y = promien * cos(alfa+PI/M);
								glNormal3f((sin(alfa + PI / M) * cos(beta)), (sin(alfa + PI / M) * sin(beta)), cos(alfa + PI / M));
        glVertex3f(x, y, z);
        x = promien * sin(alfa) * cos(beta);
        z = promien * sin(alfa) * sin(beta);
        y = promien * cos(alfa);
								glNormal3f((sin(alfa) * cos(beta)), (sin(alfa) * sin(beta)), cos(alfa));
        glVertex3f(x, y, z);
    }
    glEnd();
  }
